...
class UpdateViewGuildsAggregates extends AbstractMigration
{
    const VIEW_CREATION = <<<'EOT'
CREATE OR REPLACE VIEW guilds_aggregates AS
SELECT guilds.id, guilds.name, guilds_domains.domain, guilds_domains.is_active as is_domain_active, guilds.owner_id, guilds.joined_date,
...
GROUP BY guilds.id, guilds.name, guilds_domains.domain, guilds_domains.is_active;
EOT;
    ...
    {
        $this->execute('DROP VIEW guilds_aggregates');
        $this->execute(ViewGuildsAggregates::VIEW_CREATION);
    }
